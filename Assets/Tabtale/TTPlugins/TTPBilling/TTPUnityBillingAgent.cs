...
namespace Tabtale.TTPlugins
{
    ...
    public class TTPUnityBillingAgent : TTPIStoreListener
    {
        ...
        private Action<string, float, string, string, Dictionary<string, object>, bool> _validatePurchaseAction;
        ...
        public TTPUnityBillingAgent(Action<PurchaseIAPResult> onPurchasedAction,
            Action<string, float, string, string, Dictionary<string, object>, bool> validatePurchaseAction, Action<bool> reportNoAdsPurchasedAction,
        ...
        private static Dictionary<string, object> SplitPurchaseTokenIntoFragments(string purchaseToken)
        {
            Dictionary<string, object> fragments = new Dictionary<string, object>();
            if (purchaseToken != null)
            {
                var originalTransactionIdPostDataIsEmpty = true;
                for (int i = 0, pos = 0; pos < purchaseToken.Length; ++i, pos += IAPS_ANDROID_TOKEN_FRAGMENT_LEN)
                {
                    ...
                    fragments.Add(tokenFragmentKey, tokenFragment);
                    originalTransactionIdPostDataIsEmpty = false;
                }
            }
            else
            {
                fragments["originalTransactionID_1"] = "null";
            }
            return fragments;
        }
        
        private Dictionary<string, object> createAndroidSubscriptionStartedPostData(string purchaseToken, Dictionary<String,object> fragments, TTPIPurchaseEventArgs e, SubscriptionInfo subscriptionInfo)
        {
            ...
            postData = new Dictionary<string, object>
            {
                ...
                {"purchasedProductID", NAIfNull(e.purchasedProduct.definition.id)},
                {"transactionID", NAIfNull(e.purchasedProduct.transactionID)},
                {"revenue",e.purchasedProduct.metadata.localizedPrice},
                {"revenueCurrency", NAIfNull(e.purchasedProduct.metadata.isoCurrencyCode)},
                {"isRestoreInProgress", _isRestoreInProgress ? 1 : 0}
            };
            ...
            foreach (KeyValuePair<string, object> pair in fragments) postData.Add(pair.Key, pair.Value);
            return postData;
        }
        ...
        public TTPIPurchaseProcessingResult ProcessPurchase(TTPIPurchaseEventArgs e)
        {
            ...
            var fragments = SplitPurchaseTokenIntoFragments(purchaseToken);
#endif
            ...
            if (_isRestoreInProgress)
            {
                ...
                if (purchaseToken != null)
                {
                    if (e.purchasedProduct.definition.type == TTPIProductType.Subscription)
                    {
                        ...
                        {
                            postData = createAndroidSubscriptionStartedPostData(purchaseToken, fragments, e,
                                subscriptionInfo);
                            _validatePurchaseAction.Invoke(purchaseToken, _iapInValidation.localizedPrice,
                                _iapInValidation.currency, _iapInValidation.id, postData, false);
                            //subscription started event will be sent after transaction validation
                        }
                    }
                }
#endif
            ...
            if(e.purchasedProduct.definition.type == TTPIProductType.Subscription)
            {
                ...
                if(subscriptionInfo != null && subscriptionInfo.isSubscribed() == Result.True)
                {
                    postData = createAndroidSubscriptionStartedPostData(purchaseToken, fragments, e,
                        subscriptionInfo);
                    // subscription started post data is filled and the event will be sent after transaction validation
                }
            }
#else
                ...
                if (ttpSubscriptionInfo != null && ttpSubscriptionInfo.isSubscribed() == Result.True)
                {
                    ...
                    if (receiptData != null)
                    {
                        ...
                        if (appleReceipt != null)
                        {
                            ...
                            {
                                if (currentIAPReceipt.transactionID ==
                                    currentIAPReceipt.originalTransactionIdentifier)
                                {
                                    ...
                                    postData = new Dictionary<string, object>
                                    {
                                        { "trialPeriod", trialPeriod },
                                        { "purchasedProductID", NAIfNull(e.purchasedProduct.definition.id) },
                                        { "transactionID", NAIfNull(currentIAPReceipt.transactionID) },
                                        {
                                            "originalTransactionID",
                                            NAIfNull(currentIAPReceipt.originalTransactionIdentifier)
                                        },
                                        { "revenue", e.purchasedProduct.metadata.localizedPrice },
                                        { "revenueCurrency", NAIfNull(e.purchasedProduct.metadata.isoCurrencyCode) }
                                    };
                                ...
                                {
                                    Debug.Log(
                                        "ProcessPurchase : transaction id does not equal original. will not send data.");
                                }
                            }
                        }
                        else
            ...
            if (purchaseToken != null)
            {
                ...
                _validatePurchaseAction.Invoke(purchaseToken, _iapInValidation.localizedPrice,
                    _iapInValidation.currency, _iapInValidation.id, postData, true);
            }
            else
            {
...
private string NAIfNull(string val)
{
    return val ?? "NA";
        ...
        {
            Debug.Log("ProcessPurchase::LogSubscriptionStarted: ");
            LogEventByReflection((ANALYTICS_TARGET_DELTA_DNA | ANALYTICS_TARGET_FIREBASE),
                TTPEvents.SUBSCRIPTION_STARTED, postData);
        }
