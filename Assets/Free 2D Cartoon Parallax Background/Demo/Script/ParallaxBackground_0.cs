...
public class ParallaxBackground_0 : MonoBehaviour
{
    ...
    private float[] startPosY = new float[7]; // Array for storing initial Y positions
    ...
    {
        get => Camera_Move;
        set => Camera_Move = value;
    }

    void Start()
    {
        _camera = transform; // Camera.main.transform;
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            startPosY[i] = Layer_Objects[i].transform.position.y; // Store initial Y position
        }
    }
    ...
    void Update()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            Layer_Objects[i].transform.position =
                new Vector2(startPos[i] + distance, startPosY[i]); // Use stored Y position
