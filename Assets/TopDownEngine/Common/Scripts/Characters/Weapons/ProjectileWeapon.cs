...
namespace MoreMountains.TopDownEngine
{
	...
	public class ProjectileWeapon : Weapon, MMEventListener<TopDownEngineEvent>
	{
		...
		private DamageUpgradeManager damageUpgradeManager;
		private CriticalUpgradeManager criticalUpgradeManager;
		private DamageMultiplierOnLowHealth damageMultiplierOnLowHealth;
		private DamageUpgradeONWave damageUpgradeONWave;
		private LifeStealUpgradeManager lifeStealUpgradeManager;
		...
		public override void Initialization()
		{
			...
			if (!_poolInitialized)
			{
				...
				damageUpgradeManager = Owner.GetComponent<DamageUpgradeManager>();
				criticalUpgradeManager = Owner.GetComponent<CriticalUpgradeManager>();
				damageMultiplierOnLowHealth = Owner.GetComponent<DamageMultiplierOnLowHealth>();
				damageUpgradeONWave = Owner.GetComponent<DamageUpgradeONWave>();
				lifeStealUpgradeManager = Owner.GetComponent<LifeStealUpgradeManager>();
			}
		}
		...
		public virtual GameObject SpawnProjectile(Vector3 spawnPosition, int projectileIndex, int totalProjectiles, bool triggerObjectActivation = true)
		{
			...
			if (projectile != null )
			{
				...
				{
					if(damageUpgradeManager is not null)
						damageUpgradeManager.ApplyDamageUpgrade(damageComponent);
					if(criticalUpgradeManager is not null)
						criticalUpgradeManager.ApplyCriticalUpgrade(damageComponent);
					if(damageMultiplierOnLowHealth is not null)
						damageMultiplierOnLowHealth.ApplyDamageUpgrade(damageComponent);
					if(damageUpgradeONWave is not null)
						damageUpgradeONWave.ApplyDamageUpgrade(damageComponent);
					if(lifeStealUpgradeManager is not null)
						lifeStealUpgradeManager.ApplyLifeStealUpgrade(damageComponent);
				}
			}
