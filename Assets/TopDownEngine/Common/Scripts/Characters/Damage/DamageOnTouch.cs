...
namespace MoreMountains.TopDownEngine
{
	...
	public class DamageOnTouch : MMMonoBehaviour
	{
		...
		[Tooltip("La probabilidad (en %) de que ocurra un golpe crítico.")]
		public float CriticalChancePercentage = 0f;
		[Tooltip("El daño crítico base sin mejoras (ejemplo: 2 significa el doble del daño base).")]
		public float CriticalDamageBase = 2f;
		[Tooltip("El incremento en porcentaje del daño crítico acumulado por mejoras (ejemplo: 10% significa un incremento del 10% adicional).")]
		public float CriticalDamageUpgradePercentage = 0f;
		
		private DamageUpgradeManager _damageUpgradeManager;

		// Variables para almacenar los valores originales de daño
		[SerializeField] public float _originalMinDamage = 10;
		[SerializeField] public  float _originalMaxDamage = 10;
		
		[Header("Life Steal")]
		[Tooltip("Habilitar o deshabilitar la funcionalidad de robo de vida")]
		...
		public float CriticalDamageMultiplier
		{
			get
			{
				// Incrementa el multiplicador base basado en el porcentaje configurado
				return CriticalDamageBase * (1 + CriticalDamageUpgradePercentage / 100f);
			}
		}
		
		[Tooltip("Porcentaje del daño infligido que se convertirá en vida recuperada")]
		[Range(0f, 100f)]
		public float LifeStealPercentage = 50f;
		...
		{
			Initialization();
			if (_originalMinDamage == 0)
			{
				_originalMinDamage = 10;
			}
			
			if(_originalMaxDamage == 0)
			{
				_originalMaxDamage = 10;
			}

			CriticalDamageBase = 2;
			MinDamageCaused = _originalMinDamage;
			MaxDamageCaused = _originalMaxDamage;
			_damageUpgradeManager = GetComponent<DamageUpgradeManager>();
			// Aplicamos la mejora de daño al inicio (si existe DamageUpgradeManager)
			if (_damageUpgradeManager != null)
			{
				_damageUpgradeManager.ApplyDamageUpgrade(this);
			}
		...
		protected virtual void OnCollideWithDamageable(Health health)
		{
			...
			Debug.Log($"Min damage caused: {MinDamageCaused}, Max damage caused: {MaxDamageCaused}");
			if (health.CanTakeDamageThisFrame())
			{
				...
				// Comprobar probabilidad de crítico
				bool isCriticalHit = UnityEngine.Random.Range(0f, 100f) < CriticalChancePercentage;
				if (isCriticalHit)
				{
					randomDamage *= CriticalDamageMultiplier;
					Debug.Log("¡Golpe crítico!");
				}
				
				// Aplica feedbacks visuales/sonoros para golpe crítico
				if (isCriticalHit)
				{
					//HitCriticalFeedback?.PlayFeedbacks(this.transform.position);
				}
				
				// Robar vida (aplicar curación al dueño del proyectil)
				if (LifeStealEnabled)
				{
					GameObject owner = null;

					// Recupera el dueño del proyectil (si el daño viene de un proyectil)
					Projectile projectile = GetComponent<Projectile>();
					if (projectile != null)
					{
						owner = projectile.SourceWeapon?.Owner.gameObject;
					}
					else
					{
						owner = Owner; // Usa el estándar cuando no es un proyectil
					}

					// Aplica curación al dueño, si existe
					if (owner != null)
					{
						Health ownerHealth = owner.GetComponent<Health>();
						if (ownerHealth != null)
						{
							// Ajuste para porcentaje (usamos LifeStealPercentage / 100f)
							float lifeToSteal = randomDamage * (LifeStealPercentage / 100f);
							ownerHealth.ReceiveHealth(lifeToSteal, owner); // Usamos el método ReceiveHealth aquí
							Debug.Log($"El propietario {owner.name} ha robado {lifeToSteal} puntos de vida.");
						}
					}
				...
				else
				{
					_colliderHealth.Damage(randomDamage, gameObject, InvincibilityDuration, InvincibilityDuration, 
						_damageDirection, TypedDamages, isCriticalHit);
				}
			}
